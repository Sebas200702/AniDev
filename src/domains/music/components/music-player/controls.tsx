...
import { RepeatButton } from '@music/components/music-player/button/repeat-button'
import { ShuffleButton } from '@music/components/music-player/button/shuffle-button'
...
export const CustomControls = ({ muted, volume }: Props) => {
  ...
  return (
    <Controls.Root className="mt-4 hidden w-full flex-col gap-2 p-4 md:flex">
      <Controls.Group className="flex items-center justify-center gap-2">
        <ShuffleButton iconClassName="h-6 w-6" />
        <NextPrevButton direction="Prev" iconClassName="h-6 w-6" />

        <PlayButton className="bg-enfasisColor cursor-pointer rounded-full p-2 shadow-lg transition-transform hover:scale-[1.1] active:scale-[0.95]">
          {isPlaying ? (
        ...
        <NextPrevButton direction="Next" iconClassName="h-6 w-6" />
        <RepeatButton iconClassName="h-6 w-6" />
      </Controls.Group>
      ...
      <Controls.Group className="flex flex-row justify-between">
        ...
        <div className="flex items-center gap-2">
          <PIPButton className="cursor-pointer p-2" />

          <button
            onClick={() =>
              navigate(
                `/music/${normalizeString(currentSong.song_title ?? 'Unknown Song')}_${currentSong.theme_id}`
              )
            }
